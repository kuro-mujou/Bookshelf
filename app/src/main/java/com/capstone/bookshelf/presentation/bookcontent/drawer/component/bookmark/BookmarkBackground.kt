    ...
    Box(
        modifier = modifier
            .drawWithCache {
                ...
                val allCircleData = mutableListOf<Pair<Offset, Float>>()
                fun createDenseClouds(center: Offset, scale: Float){
                    ...
                    parts.forEach { partOffset ->
                        ...
                        allCircleData.add(Pair(circleCenter, circleRadius))
                    }
                }
                val cloudY = height
                var x = -40f
                while (x < width + 40f) {
                    ...
                    val cloudCenter = Offset(x, cloudY)
                ...
                fun createBirdPaths(): List<Pair<Path, Stroke>> {
                    ...
                    repeat(birdCount) {
                        ...
                        val centerY = height * (0.2f + Random.nextFloat() * 0.3f)
                ...
                val birdPaths = createBirdPaths()
                onDrawBehind {
                    allCircleData.forEach { (center, radius) ->
                        drawCircle(
                            color = baseColor,
                            center = center,
                            radius = radius
            ...
            .drawWithCache {
                ...
                fun createWavePath(
                    ...
                    while (currentX < width + 100f) {
                        ...
                        path.quadraticTo(
                            controlX.coerceIn(0f, width),
                            controlY,
                            nextX.coerceIn(0f, width),
                            endY
                        )
                        currentX = nextX
                    }
                ...
                val wavePath1 = createWavePath(30f, -10f..10f, 30f..60f)
                val wavePath2 = createWavePath(50f, -12f..12f, 40f..70f)
                val wavePath3 = createWavePath(70f, -15f..15f, 50f..80f)
                val birdDrawables = createBirdPaths()
                onDrawBehind {
                    drawPath(wavePath1, color = baseColor)
                    drawPath(wavePath2, color = baseColor.copy(alpha = 0.6f))
                    drawPath(wavePath3, color = baseColor.copy(alpha = 0.3f))
                    birdDrawables.forEach { (path, stroke) ->
                        drawPath(
                            path = path,
                            color = baseColor.copy(0.8f),
                            style = stroke
                        )
                    }
                }
                ...
                onDrawBehind {
                    ...
                    bigStars.forEach { (center, size, alpha) ->
                        val path = Path().apply {
                            ...
                            quadraticTo(center.x + size * 0.1f, center.y - size * 0.1f, center.x + size, center.y)
                            quadraticTo(center.x + size * 0.1f, center.y + size * 0.1f, center.x, center.y + size)
                            quadraticTo(center.x - size * 0.1f, center.y + size * 0.1f, center.x - size, center.y)
                            quadraticTo(center.x - size * 0.1f, center.y - size * 0.1f, center.x, center.y - size)
                            close()
                        }
    ...
    Box(
        modifier = modifier
            .drawWithCache {
                ...
                onDrawBehind {
                    trianglesToDraw.forEach { (path, color) ->
                        drawPath(
                            path = path,
                            color = color,
                            style = Fill
    ...
    Box(
        modifier = modifier
            .drawWithCache {
                val width = size.width
                val height = size.height
                val minRadiusPx = minRadius.toPx()
                ...
                val hexagons = List(hexagonCount) { i ->
                ...
                val strokeStyle = Stroke(width = 1.5.dp.toPx(), cap = StrokeCap.Round, join = StrokeJoin.Round)
                onDrawBehind {
    ...
    Box(
        modifier = modifier
            .drawWithCache {
                ...
                val (baseHue, baseSat, baseVal) = baseColor.toHsv()
                val hexagonsToDraw = mutableListOf<Pair<Path, Color>>()
                var hexCounter = 0
                ...
                val endY = height + patternRepeatHeight + radiusPx
                val endX = width + hexWidth + (hexWidth / 2f)
                var row = 0
                var y = startY
                while (y < endY) {
                    ...
                    while (x < endX) {
                        ...
                        val randomOutlineColor = Color.hsv(baseHue, randomSat, randomVal, randomAlpha)
                        hexCounter++
                        hexagonsToDraw.add(Pair(path, randomOutlineColor))
                        x += hexWidth
                    }
                    y += (2 * radiusPx * 3 / 4)
                    row++
                ...
                val strokeStyle = Stroke(width = 1.5.dp.toPx(), cap = StrokeCap.Round, join = StrokeJoin.Round)
                onDrawBehind {
                    translate(left = -randomOffsetX, top = -randomOffsetY) {
                        hexagonsToDraw.forEach { (path, outlineColor) ->
                            drawPath(
                                path = path,
                                color = outlineColor,
                                style = strokeStyle
    ...
    Box(
        modifier = modifier
            .drawWithCache {
                val width = size.width
                val height = size.height
                val petalColor: Color = baseColor.lighten(0.2f).copy(alpha = 0.8f)
                ...
                val unitPetalPath = Path().apply {
                    ...
                    val maxWidth = nominalWidth
                    val leftTipX = -tipWidth / 2f
                    val rightTipX = tipWidth / 2f
                    val controlPointHorizontalOffset = maxWidth * 0.55f
                    val controlPointVerticalOffsetNearTip = nominalHeight * 0.3f
                    val controlPointVerticalOffsetNearBase = nominalHeight * 0.4f
                    moveTo(0f, baseY)
                    cubicTo(
                        -controlPointHorizontalOffset, baseY - controlPointVerticalOffsetNearBase,
                        leftTipX - controlPointHorizontalOffset * 0.1f, tipY + controlPointVerticalOffsetNearTip,
                        leftTipX, tipY
                    )
                    lineTo(0f, notchCenterY)
                    lineTo(rightTipX, tipY)
                    cubicTo(
                        rightTipX + controlPointHorizontalOffset * 0.1f, tipY + controlPointVerticalOffsetNearTip,
                        controlPointHorizontalOffset, baseY - controlPointVerticalOffsetNearBase,
                        0f, baseY
                    )
                    close()
